...
class SPECommon {
	...
	private static $SP_enquiry_url;
	private static $SP_enquiry_dir;

	private static $SP_data_queue = array();

	public static function set_enquiry_url($base) {
	...
	public static function set_enquiry_dir($base) {
	...
	public static function enquiry_url($extended) {
		return self::$SP_enquiry_url . $extended;
	}

	public static function enquiryaddons_url($extended) {
		return self::enquiry_url( 'addons/' . $extended);
	}

	// Returns the property plugin directory
	public static function enquiry_dir($extended) {
		return self::$SP_enquiry_dir . $extended;
	}

	public static function enquiryaddons_dir($extended) {
		return self::enquiry_dir( 'addons/' . $extended);
	}
	...
	public static function load_enquiry_addons() {

		$plugins =  self::_get_enquiry_plugins( );

		if(!empty($plugins)) {
			foreach($plugins as $file => $plugin) {
				include_once( self::enquiryaddons_dir( $file ) );
			}
		}

	}

	private static function _get_enquiry_plugins($plugin_folder = 'addons') {

		if ( ! $cache_plugins = wp_cache_get('enquiry_plugins', 'enquiry_plugins') )
			$cache_plugins = array();
		...
		$sp_plugins = array ();
		$plugin_root = self::enquiry_dir( $plugin_folder );
		...
		wp_cache_set('enquiry_plugins', $cache_plugins, 'enquiry_plugins');

		return $sp_plugins;
	}

	private static function _get_enquiry_plugin_data( $plugin_file, $markup = true, $translate = true ) {
